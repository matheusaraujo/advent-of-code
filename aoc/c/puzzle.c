...
struct Puzzle read_puzzle(const char *file_path)
{
    ...
    if (file_content == NULL || strlen(file_content) == 0)
    {
        ...
        puzzle.title = NULL;
        puzzle.full_title = NULL;
        puzzle.year = NULL;
        puzzle.day = NULL;
        puzzle.inputFile = NULL;
        puzzle.part1_outputFile = NULL;
        puzzle.part2_outputFile = NULL;
        return puzzle;
    }

    puzzle.initialized = 1;
    puzzle.title = extract_value(file_content, "\"title\": \"", "\"");
    puzzle.full_title = extract_value(file_content, "\"fullTitle\": \"", "\"");
